/****************************************************************************
 ...
 "EPPS21: Nuclear parton distributions with LHC data."
 Published in EPJC
 Eprint: arXiv:2112.12462
 ...
 ***************************************************************************
 ...
 For given input values of
     ...
     order (integer): dummy variable, retained for
                      compatibility with EPPS16 routine
 ...
 the function (returning void)
   ...
   EPPS21(order, pset, A, x, Q, ruv, rdv, ru, rd, rs, rc, rb, rg)
 ...
 in Secs. 4.2 and 4.3 of arXiv:2112.12462.
             ...
             double       &rg)
{
     ...
     static int Alast=-1;
     static double allvalues[108][9][51][251];
...
// *********************************************
  ...
  if (pset < 1 || pset > 107){
      cout << "In EPPS21: Wrong set!" << " " << pset << endl;
      cout << "Central set: pset = 1" << endl;
      cout << "Error sets : pset = 2...107" << endl;
...
// *******************************
      ...
      if (x<x_i){
      cout << "In EPPS21: x<1E-7" << endl;
      std::exit(EXIT_FAILURE);
      }
      if (x > 1){
      cout << "In EPPS21: x>1" << endl;
      std::exit(EXIT_FAILURE);
      }
...
// ************************************
      ...
      if (Q2 < Q2min){
      cout << "In EPPS21: Q<1.3GeV" << endl;
      std::exit(EXIT_FAILURE);
      }
      if (Q2 > Q2max){
      cout << "In EPPS21: Q>10000GeV" << endl;
      std::exit(EXIT_FAILURE);
      }
...
// If the set specifications have been changed, read the tables again
 ...
 if (A != Alast){
  sprintf(filenimi,"EPPS21NLOR_%d", A);
  
  fstream myfile;

  myfile.open (filenimi, ios::in);
  if (!myfile.is_open()){
  cout << "In EPPS21: Missing file " << filenimi << endl;
  exit(EXIT_FAILURE);
  }

  for (int setnumber=1;setnumber<108;setnumber++){
...
// Find the position in the x-grid
      ...
      LSTEP = (0.0 - (log(1.0/x_i) + 5.0*(1.0-x_i)))/(1.0*xsteps);

      n_x  = ((log(1.0/x)+5.0*(1-x))-(log(1.0/x_i)+5.0*(1.0-x_i)))/LSTEP;
      xpoint = n_x;
